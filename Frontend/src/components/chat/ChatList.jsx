...
const ChatList = () => {
  ...
  useEffect(() => {
    ...
    const removeUserStatusListener = onEvent('userStatus', ({ userId, status }) => {
      setOnlineUsers(prev => {
    ...
    const removeNewMessageListener = onEvent('newMessage', (message) => {
      setChats(prevChats => {
        ...
        const chatIndex = updatedChats.findIndex(
          chat => chat.user._id === message.sender._id
        );
    ...
    const removeMessagesReadListener = onEvent('messagesRead', ({ reader, messageIds }) => {
      if (reader !== user._id) {
        setChats(prevChats => {
          return prevChats.map(chat => {
            if (chat.lastMessage && messageIds.includes(chat.lastMessage._id)) {
              return {
                ...chat,
                lastMessage: {
                  ...chat.lastMessage,
                      ...
                      <div className="flex justify-between items-baseline">
                        ...
                        {chat.lastMessage && (
                          <span className="text-xs text-gray-500">
                            {format(new Date(chat.lastMessage.createdAt), 'MMM dd')}
                          </span>
                        )}
                      </div>
                      
                      <div className="flex justify-between items-center">
                        <p className={`text-sm ${chat.lastMessage?.unread ? 'text-gray-900 font-medium' : 'text-gray-500'} truncate`}>
